...
<br>
<font color="red"><strong>ALERT:</strong> This household has been detected to be out-of-district and does not have an appropriate assessment. To continue, you must go to the <strong>Assessments</strong> section at left and obtain either quarterly assessment or an annual assessment that covers the league start and end dates.</font></strong>
...
<cfelseif GetAppTypeLeagueFees.recordcount gt 0>

<form name="f" method="POST" action="processleague2.cfm">
...
<tr>
	...
	<td rowspan="4">&nbsp;</td>
	<td valign="middle">#GetPatrons.lastname#, #GetPatrons.firstname#
		<input type="checkbox" name="selectedpatrons" value="#GetPatrons.secondarypatronid[GetPatrons.currentrow]#">	
	</td>
</tr>
<tr>
	...
	<td valign="middle">
		...
		<select name="SelectAppType#GetPatrons.secondarypatronid[GetPatrons.currentrow]#" class="form_input">
			<option selected value="0">#StringPad("Select League and Rate", 20)#
               ...
               <CFSET thispatronfee = getfee.val>
				...
				<cfif (GetPatrons.gender[GetPatrons.currentrow] is "M" and FindNoCase("Boy", description) gt 0) or (GetPatrons.gender[GetPatrons.currentrow] is "F" and FindNoCase("Girl", description) gt 0)>
					...
					<CFIF datecompare(GetPatrons.dob[GetPatrons.currentrow],mindob) LT 0>
                         	<option value="0">#description# (Patron exceeds age max.)
                              
                         <CFELSEIF datecompare(GetPatrons.dob[GetPatrons.currentrow],maxdob) GT 0>
                         	<option value="0">#description# (Patron does not meet age min.)

					<CFELSE>

					<!--- check enrollment vs. maxqty --->
					<cfif enrolledcount lt maxqty>
						<option value="#typecode#">#description# (#DollarFormat(thispatronfee)#<cfif IsInDevMode> - Max: #maxqty#</cfif>) <CFIF offershirt EQ 0>* No Shirt</CFIF>
					<cfelse>
						<option value="0">#description# (full - contact #facid#<cfif IsInDevMode> - Max: #maxqty#</cfif>)
					</cfif>
...
<cfif OfferShirts is 1>
	<tr>
		...
		<td valign="middle">
		<select name="selectshirt#GetPatrons.secondarypatronid[GetPatrons.currentrow]#" class="form_input">
				<option value="NA">#StringPad("Select Shirt", 4)#
				<cfloop query="GetLeaguePatronShirtSizes">
					<option value="#sizecode#">#sizedescription#
				</cfloop>
                    <option  value="NA">No Shirt
...
<cfelse>
	<input name="selectshirt#GetPatrons.secondarypatronid[GetPatrons.currentrow]#" type="Hidden" value="NA">
...
<tr>
	...
	<td valign="middle">
		<select name="selectschool#GetPatrons.secondarypatronid[GetPatrons.currentrow]#" class="form_input">
		<option value="0" >#StringPad("School Path Options", 27)#

		<cfloop query="GetSchools1">
			<option value="#rn#"><cfif schoolname is not "ISB">#schoolname# Elementary -> </cfif>#middle# Middle -> #high#
		</cfloop>
