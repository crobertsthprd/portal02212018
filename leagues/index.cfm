...
function setshirt() {
	...
	var e = document.getElementById("theLeague");
	//var s = document.getElementById("theShirt");
	var leagueString = e.options[e.selectedIndex].value;
	var leagueArray = leagueString.split("^");
	offerShirt = leagueArray[leagueArray.length - 1];
	if (offerShirt == "0") {
		document.getElementById("theShirt").selectedIndex = 0;
		alert("Selected league does not have team shirts.");
	}
}
</SCRIPT>    
...
<tr>
	...
	<td valign="middle">
		...
		<select name="SelectAppType" class="form_input" id="theLeague" onChange="setshirt();">
...
<cfif OfferShirts is 1>
	<tr>
		...
		<td valign="middle">
		<select name="selectshirt" class="form_input" id="theShirt">
				<option  value="#GetPatrons.secondarypatronid[GetPatrons.currentrow]#^NA">No Shirt
                    <option selected value="#GetPatrons.secondarypatronid[GetPatrons.currentrow]#^N/A">#StringPad("Select Shirt", 4)#
