...
<!--- new functions move here from application.cfc on 08/25/2016 --->

<cffunction name="ccformat" output="true" description="Return cc formatted string" returntype="String">
<cfargument name="invar" default="" required="no">
<cfset var formattedstr = REReplace( arguments.invar, "[^0-9]", "", "all" )>

<cfif len( arguments.invar ) eq 16>
	<cfset local.formattedstr = mid( local.formattedstr, 1, 4 ) & " " & mid( local.formattedstr, 5, 4 ) & " " & mid( local.formattedstr, 9, 4 ) & " " & mid( local.formattedstr, 13, 4 )>
</cfif>

<cfreturn local.formattedstr>
</cffunction>
...
<cffunction name="hasopencall" output="true" description="Returns true if specified session or primarypatron has an open credit card call" returntype="boolean">
<cfargument name="thissession" type="string" required="false" default="">
<cfset opencallquery = "" />

<cfif arguments.thissession eq "">
	<cfreturn false>
</cfif>

<!--- switches between primarypatronid (if numeric) and sessionid for inspection --->
<cfif IsNumeric( arguments.thissession )>
	<!--- get session for this primary --->
	<CFQUERY name="local.opencallquery" datasource="#application.dopsds#">
		SELECT   sessionid
		FROM     dops.sessionpatrons
		WHERE    primarypatronid = <cfqueryparam value="#arguments.thissession#" cfsqltype="cf_sql_integer" list="no">
		AND      relationtype = <cfqueryparam value="1" cfsqltype="cf_sql_smallint" list="no">
	</CFQUERY>

	<!--- if opencallquery failed --->
	<cfif local.opencallquery.recordcount eq 0>
		<cfreturn false>
	</cfif>

	<!--- check for open call --->
	<CFQUERY name="local.opencallquery" datasource="#application.dopsds#">
		select   dops.hasopencall( <cfqueryparam value="#local.opencallquery.sessionid#" cfsqltype="cf_sql_varchar" list="no"> ) as call
	</CFQUERY>

	<cfreturn local.opencallquery.call>
<cfelse>

	<!--- check for open call --->
	<CFQUERY name="local.opencallquery" datasource="#application.dopsds#">
		select   dops.hasopencall( <cfqueryparam value="#arguments.thissession#" cfsqltype="cf_sql_varchar" list="no"> ) as call
	</CFQUERY>
...
<cffunction name="sessionactivity" output="true" description="Returns session activity from supplied primarypatronid" returntype="string">
<cfargument name="thisprimary" type="string" required="false" default="">
<cfset activityquery = "" />

<cfif val( arguments.thisprimary ) eq 0>
	<cfreturn "">
</cfif>

<!--- check for registration activity --->
<CFQUERY name="local.activityquery" datasource="#application.dopsds#">
	select   reg.pk
	from     dops.reg
	where    reg.primarypatronid = <cfqueryparam value="#arguments.thisprimary#" cfsqltype="cf_sql_integer" list="no">
	and      reg.sessionid is not null
	limit    1
</CFQUERY>

<cfif local.activityquery.recordcount eq 1>
	<cfreturn "REG">
</cfif>

<!--- check for pass activity --->
<CFQUERY name="local.activityquery" datasource="#application.dopsds#">
	select   sessionassessments.pk
	from     dops.sessionassessments
	where    sessionassessments.primarypatronid = <cfqueryparam value="#arguments.thisprimary#" cfsqltype="cf_sql_integer" list="no">
	limit    1
</CFQUERY>
...
<CFQUERY name="local.activityquery" datasource="#application.dopsds#">
	...
	where    sessionpasses.primarypatronid = <cfqueryparam value="#arguments.thisprimary#" cfsqltype="cf_sql_integer" list="no">
	limit    1
</CFQUERY>
...
<!--- return nothing if no session activity found --->
<cfreturn "">
...
<cffunction name="sessionisapproved" output="true" description="Returns true if specified session or primarypatron has an unatached approved credit card call" returntype="boolean">
<cfargument name="thissession" type="string" required="false" default="">
<cfset opencallquery = "" />
...
<cfif IsNumeric( arguments.thissession )>
	...
	<!--- check for approved call --->
	<CFQUERY name="local.opencallquery" datasource="#application.dopsds#">
		select   invoicetranxcall.pk
		from     dops.invoicetranxcall
		where    invoicetranxcall.sessionid = <cfqueryparam value="#local.opencallquery.sessionid#" cfsqltype="cf_sql_varchar" list="no">
		and      dops.istranxcallapproved2( invoicetranxcall.procname, invoicetranxcall.page, invoicetranxcall.istest, false )
		limit    1
	</CFQUERY>

	<cfreturn local.opencallquery.recordcount eq 1>
<cfelse>
	...
	<CFQUERY name="local.opencallquery" datasource="#application.dopsds#">
		select   invoicetranxcall.pk
		from     dops.invoicetranxcall
		where    invoicetranxcall.sessionid = <cfqueryparam value="#arguments.thissession#" cfsqltype="cf_sql_varchar" list="no">
		and      dops.istranxcallapproved2( invoicetranxcall.procname, invoicetranxcall.page, invoicetranxcall.istest, false )
	</CFQUERY>
...
<cffunction name="sessionlock" output="true" description="Returns true is specified session is locked via dops.sessionlock" returntype="boolean">
<cfargument name="invar" required="yes">
<cfset var sessioncheck = "">

<CFQUERY name="sessioncheck" datasource="#application.dopsds#">
	Select   sessionid
	from     dops.sessionlock
	where    sessionid = <cfqueryparam value="#arguments.invar#" cfsqltype="cf_sql_varchar" list="no">
	limit    1
</CFQUERY>

<cfif sessioncheck.recordcount eq 1>
	<cfreturn true>
<cfelse>
	<cfreturn false>
</cfif>
