...
<CFIF Isdefined("form.cardnumber2reg")>
	...
	<cfquery datasource="#application.reg_dsn#" name="GetOtherCreditBalance">
		...
		FROM     dops.othercredithistorysums othercredithistorysums
	...
	<cfquery datasource="#application.reg_dsn#" name="CheckForCardInStock">
		select   pk
		from     dops.othercreditdatastock
	...
	<CFIF errormsg EQ "">
		...
		<cfquery datasource="#application.reg_dsn#" name="UpdateCard">
			update   dops.othercreditdata
			set
				primarypatronid = <cfqueryparam value="#cookie.uid#" cfsqltype="cf_sql_integer" list="no">
			where cardid = <cfqueryparam value="#CheckCardStatus.cardid#" cfsqltype="cf_sql_integer" list="no">
			;
			insert into dops.othercreditdatahistory
				...
				module)
			values
				(<cfqueryparam value="#CheckCardStatus.cardid#" cfsqltype="cf_sql_integer" list="no">,
				<cfqueryparam value="#LocalFac#" cfsqltype="cf_sql_varchar" list="no">,
				<cfqueryparam value="S" cfsqltype="cf_sql_char" maxlength="1" list="no">,
				<cfqueryparam value="0" cfsqltype="cf_sql_integer" list="no">,
				<cfqueryparam value="#module#" cfsqltype="cf_sql_varchar" list="no">)
		</cfquery>
		</cftransaction>
   		...
   		<table border=0 cellpadding=2 cellspacing=0 width=749>
			...
			<tr>
				...
				<table border="0" cellpadding="3" cellspacing="0" width="100%">
					...
					<cfquery datasource="#application.reg_dsn#" name="getAct">
						select   h.dt
						from     dops.othercreditdatahistory h
						where    h.cardid = <cfqueryparam value="#getcards.cardid#" cfsqltype="cf_sql_integer" list="no">
						and      h.action = <cfqueryparam value="S" cfsqltype="cf_sql_char" maxlength="1" list="no">
						order by h.dt desc
						limit 1
					</cfquery>
					...
					<tr >
						<td><strong>#left(cryp.value,4)# #insert(" ",mid(cryp.value,5,8),4)# #right(cryp.value,4)#</strong>
                              ...
                              <a href="giftcardsregister.cfm?cardnamenumber=#cryp.value#"><CFIF trim(getcards.cardname) NEQ "">Rename<CFELSE>Name</CFIF> This Card</a></td>
						<td valign="top"><CFIF isfa EQ 0>Gift Card<CFELSE>Family Assistance</CFIF></td>
						<td valign="top">$#numberformat(sumnet,"____.__")#</td>
						<td align="center" valign="top"><CFIF allowreloadflag EQ 1 AND getcards.isfa EQ 0><a href="ocinfo.cfm?reloadcardnumber=#cryp.value#">Reload</a> | </CFIF><a href="giftcardsregister.cfm?historycardnumber=#cryp.value#">View History</a></td>
					</tr>
				...
				<table border="0" cellpadding="3" cellspacing="0" width="100%">
					...
					<CFQUERY name="getcardname" dbtype="query">
						...
						<td rowspan="#getFAbalance.recordcount+1#" valign="top"><strong>#left(cryp.value,4)# #insert(" ",mid(cryp.value,5,8),4)# #right(cryp.value,4)#</strong><CFIF trim(getcards.cardname) NEQ ""> / #getcards.cardname#</CFIF><br>
                              <a href="ocinfo.cfm?cardnamenumber=#cryp.value#"><CFIF trim(getcards.cardname) NEQ "">Rename<CFELSE>Name</CFIF> This Card</a></td></CFIF>

						<td >#firstname# #lastname#<!--- <em>#secondarypatronid#---></em></td>
						<td align="right">#dollarformat(balance)#</td>
						<td align="center"><CFIF getFAbalance.currentrow EQ 1><a href="ocinfo.cfm?historycardnumber=#cryp.value#">View History</a></CFIF></td>
					</tr>
    ...
    <font color="red">ERROR: <b>Requested card could not be found.</b></font><br>
				...
				<CFLOOP from="1" to="#arraylen(variables.myCart)#" index="i">
					<tr>
						...
						<td><a href="ocinfo.cfm?removeid=#i#">Remove</a></td>
					</tr>
				</CFLOOP>
				</CFIF>
...
<cfif 0 and IsDefined("cookie.primarypatronid")>

	<cfquery datasource="#application.reg_dsn#" name="getPrimarySessionID">
		select   sessionid
		from     dops.sessionpatrons
		where    primarypatronid = <cfqueryparam value="#cookie.primarypatronid#" cfsqltype="cf_sql_integer" list="no">
		and      relationtype = <cfqueryparam value="1" cfsqltype="cf_sql_integer" list="no">
	</cfquery>

	<cfif getPrimarySessionID.recordcount eq 1>
		<p>If you would like to purchase a new gift card please visit our <a href="/storenewBP/giftcard_home.cfm?currentsessionid=#getPrimarySessionID.sessionid#" target="_blank">online store</a>.</p>
	</cfif>

<cfelse>
		<p>If you would like to purchase a new gift card please visit our <a href="/storenewBP/giftcard_home.cfm" target="_blank">online store</a>.</p>
</cfif>
