...
<CFIF DisplayMode EQ "A">
	<CFLOCATION url="/portal/assessments/index1.cfm">
</CFIF>
   ...
   <td valign=top>
	...
	<TR>
		<TD colspan="11">
     	...
     	<tr>
          	<td colspan="5" style="background:##FF9;border-width:1px;border-color:##000;border-style:solid;padding:2px;color:##000"><strong>To update an aquatic or tennis level please call the appropriate aquatic or tennis center. 'N/A' indicates that the level has not been assigned by a THPRD instructor or other staff.</strong>
				...
				<table width="100%" border="0" cellpadding=1 cellspacing=0>
					...
					<cfloop query="CheckAssmtStatus">
						...
						<CFSCRIPT>
							...
							encrypted=encrypt("#invoicefacid#-#Invoicenumber#", key, "CFMX_COMPAT", "Hex");
                         	</CFSCRIPT>
						<tr>
							<TD>#lastname#, #firstname# #middlename#</TD>
							...
							<TD>
								<a href="javascript:void(0);" onClick="window.open('/checkout/invoice/printinvoice.cfm?i=#encrypted#&p=y','receipt','toolbars=no, scrollbars=yes, resizable');">#invoicefacid#-#Invoicenumber#</a>
							</TD>
						</tr>
					</cfloop>
...
<!--- end set up for assessments --->
				...
				<CFIF getNextAssess.recordcount GT 0>
					<CFSET therefdate = dateformat(dateadd('d',-1,getNextAssess.assmtexpires),'yyyy-mm-dd')>
				</CFIF>
				...
				<CFQUERY name="getCurrentPre" datasource="#application.slavedopsds#" >
					select * from assessmentrates
					where available is true and assmteffective < #now()# and assmtexpires > #now()#
					<CFIF exclusion NEQ 0 and trim(exclusion) NEQ "">and id not in (#exclusion#)</CFIF>
				...
				<CFIF getCurrentPre.recordcount GT 0>
					<CFSET therefdate = dateformat(dateadd('d',-1,getCurrentPre.assmteffective),'yyyy-mm-dd')>
                         <CFSET testVar = dateformat(dateadd('d',-1,getCurrentPre.assmteffective),'yyyy-mm-dd')>
				</CFIF>
				...
				<!--- filter  --->
                    <CFSET thelist = listappend(valuelist(getCurrent.id),0)>

				<CFQUERY name="allOthers" dbtype="query" >
					select * from getCurrentPre

                         where id not in (#thelist#)
                         <CFIF exclusion NEQ 0 and trim(exclusion) NEQ "">and id not in (#exclusion#) </CFIF>
								...
								<cfif (CheckPassStatus.passallocation[CheckPassStatus.currentrow] is 0) or (CheckPassStatus.passallocation[CheckPassStatus.currentrow] is not 0 and CheckPassStatus.passuses[CheckPassStatus.currentrow] is not CheckPassStatus.passallocation[CheckPassStatus.currentrow])>
									...
									<cfquery datasource="#application.slavedopsds#" name="GetPatronData">
										...
										where patronid = #GetPassDetails.PatronID#
                                             ...
                                             <cfset str1 = CheckPassStatus.InvoiceFacID[CheckPassStatus.currentrow] & "-" & CheckPassStatus.InvoiceNumber[CheckPassStatus.currentrow]>
									<CFSCRIPT>
									//theKey=generateSecretKey(key);
									encrypted=encrypt("#str1#", key, "CFMX_COMPAT", "Hex");
                         				</CFSCRIPT>
									<tr >
										<TD nowrap><a href="/checkout/invoice/printinvoice.cfm?i=#encrypted#" target="_blank">#str1#</a></TD>
			...
			<cfif IsVoided is 0 or IsDefined("AcctMode")>
				<cfif InvoiceFacID is not ThisFac or InvoiceNumber is not ThisInvoice>
					...
					<CFSCRIPT>
						...
						<td class="bodytext">
							<cfif printable is 1>
								<!---<a href="javascript:void(0);" onClick="window.open('/checkout/invoice/printinvoice.cfm?i=#encrypted#&p=y','receipt','toolbars=no, scrollbars=yes, resizable');">#GetAccountHistory.InvoiceFacID#-#GetAccountHistory.InvoiceNumber#</a>--->

								<!--- new invoice system --->
								<!---<cfset variables.eip = lCase( AESencrypt( CreateUUID() & "_" & GetAccountHistory.pk & "_" & CreateUUID() ) )>--->
								<cfset variables.eip = encryptvar( GetAccountHistory.pk )>
								<a target="_blank" href="/common/printinvoice/PrintInvoice.cfm?eip=#variables.eip#">#GetAccountHistory.InvoiceFacID#-#GetAccountHistory.InvoiceNumber#</a>
							</cfif>
						</td>
...
<!---
<CFDUMP var="#getCurrentPre#" >
<CFDUMP var="#getCurrent#">
<CFDUMP var="#getNext#">
<CFDUMP var="#allothers#">
<CFOUTPUT>Refdate: #therefdate#<br>
DateAdd getCurrentPre: #dateformat(dateadd('d',-2,getCurrentPre.assmtexpires),'yyyy-mm-dd')#<br>
GetCurrentPre: #getCurrentPre.assmtexpires#<br>
Testvar: #testvar#</CFOUTPUT>

<CFDUMP var="#checkassmtstatus#">
<CFDUMP var="#getCurrentPre#">
<CFDUMP var="#getCurrent#">
--->
</CFIF>
